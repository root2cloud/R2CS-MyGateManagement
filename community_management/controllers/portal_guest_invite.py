...
class PortalGuestInvite(http.Controller):
    ...
    @http.route('/my/guest/invites/edit/<int:invite_id>', type='http', auth='user', website=True, methods=['GET', 'POST'], csrf=False)
    def portal_guest_invite_edit(self, invite_id, **post):
        partner = request.env.user.partner_id
        Invite = request.env['guest.invite'].sudo()
        invite = Invite.browse(invite_id)

        if not invite.exists() or invite.resident_id.id != partner.id:
            return request.redirect('/my/guest/invites')

        if request.httprequest.method == 'POST':
            # Similar to create, but update existing
            vals = {
                'invite_type': post.get('invitetype', invite.invite_type),
                'duration_type': post.get('durationtype', invite.duration_type),
                'is_private': bool(post.get('isprivate')),
                'note': post.get('note') or invite.note,
            }

            if vals['invite_type'] == 'once':
                vals['once_date'] = post.get('oncedate') or invite.once_date
                timestr = post.get('oncestarttime', '09:00')
                try:
                    t = datetime.strptime(timestr, '%H:%M')
                    vals['once_start_time'] = t.hour + t.minute / 60.0
                except:
                    vals['once_start_time'] = invite.once_start_time
                vals['once_valid_hours'] = int(post.get('oncevalidhours') or invite.once_valid_hours)

            if vals['invite_type'] == 'frequent':
                vals['freq_start_date'] = post.get('freqstartdate') or invite.freq_start_date
                vals['freq_end_date'] = post.get('freqenddate') or invite.freq_end_date

            guestname = post.get('guestname')
            guestmobile = post.get('guestmobile')
            if guestname and guestmobile:
                # For simplicity, replace guests (you can make it more advanced)
                invite.guest_line_ids.unlink()
                vals['guest_line_ids'] = [(0, 0, {
                    'guest_name': guestname,
                    'guest_mobile': guestmobile
                })]
        ...
        # GET: Render edit form with pre-filled values
        values = {
            'invite': invite,
            # Pre-fill form fields
            'invitetype': invite.invite_type,
            'durationtype': invite.duration_type,
            'isprivate': invite.is_private,
            'note': invite.note,
            'oncedate': invite.once_date,
            'oncestarttime': f"{int(invite.once_start_time):02d}:00",  # Approximate
            'oncevalidhours': invite.once_valid_hours,
            'freqstartdate': invite.freq_start_date,
            'freqenddate': invite.freq_end_date,
            'guestname': invite.guest_line_ids[0].guest_name if invite.guest_line_ids else '',
            'guestmobile': invite.guest_line_ids[0].guest_mobile if invite.guest_line_ids else '',
        }
        return request.render('community_management.portal_guest_invite_edit', values)  # New template below

    @http.route('/my/guest/invites/delete/<int:invite_id>', type='http', auth='user', website=True, methods=['POST'], csrf=False)
    def portal_guest_invite_delete(self, invite_id, **kw):
        Invite = request.env['guest.invite'].sudo()
        invite = Invite.browse(invite_id)

        if invite.exists() and invite.resident_id.id == request.env.user.partner_id.id:
            invite.unlink()
