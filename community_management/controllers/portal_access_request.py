...
class ResidentAccessRequestPortal(http.Controller):
    ...
    def resident_request_form(self, **kw):
        ...
        occupancy_types = request.env['resident.access.request'].fields_get(['occupancy_type'])['occupancy_type'][
            'selection']
    ...
    def resident_request_submit(self, **post):
        ...
        # === FIXED: PROPERLY HANDLE OWNER FROM TEXT INPUT ===
        owner_id = False
        owner_name = post.get('owner_id', '').strip()

        if owner_name:
            # Search for existing partner by name (case-insensitive, exact match)
            ...
            if partner:
                ...
                # If owner doesn't exist, create a new partner
        ...
        # Prepare values - ONLY CREATE REQUEST, NO PORTAL USER
        vals = {
            'name': post.get('name'),
            ...
            'owner_id': owner_id,  # Now properly set
        }

        try:
            # Create request record only - NO PORTAL USER CREATED HERE
            new_request = request.env['resident.access.request'].sudo().create(vals)
            # Submit the request (sets to pending, creates activity for approver)
